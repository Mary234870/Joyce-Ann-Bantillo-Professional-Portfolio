...
function findTwoSum(nums, target) {
    let result = [];
    // We loop from the end to ensure we get the pair with the largest index first
    for (let i = nums.length - 1; i >= 0; i--) {
        const complement = target - nums[i];
        // Check if complement exists earlier in the array
        for (let j = 0; j < i; j++) {
            if (nums[j] === complement) {
                result = [j, i]; // store the correct indices
